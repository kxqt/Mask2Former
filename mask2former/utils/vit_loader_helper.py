...
def load_weights_from_npz(model, checkpoint_path: str, prefix: str = ""):
    ...
    embed_conv_w = adapt_input_conv(
        model.patch_embed.weight.shape[1],
        _n2p(w[f"{prefix}embedding/kernel"]),
    )
    _copy("patch_embed.weight", embed_conv_w)
    _copy("patch_embed.bias", _n2p(w[f"{prefix}embedding/bias"]))
    ...
    _copy("final_norm.weight", _n2p(w[f"{prefix}Transformer/encoder_norm/scale"]))
    _copy("final_norm.bias", _n2p(w[f"{prefix}Transformer/encoder_norm/bias"]))
    # if (
    ...
    for i, block in enumerate(model.layers.children()):
        ...
        _copy(f"layers.{i}.norm1.weight", _n2p(w[f"{block_prefix}LayerNorm_0/scale"]))
        _copy(f"layers.{i}.norm1.bias", _n2p(w[f"{block_prefix}LayerNorm_0/bias"]))
        _copy(
            f"layers.{i}.attn.in_proj_weight",
            torch.cat(
                [
        ...
        _copy(f"layers.{i}.attn.out_proj.bias", _n2p(w[f"{mha_prefix}out/bias"]))
        for r in range(2):
            ...
            #     _n2p(w[f"{block_prefix}MlpBlock_3/Dense_{r}/bias"])
            # )
        _copy(f"layers.{i}.norm2.weight", _n2p(w[f"{block_prefix}LayerNorm_2/scale"]))
        _copy(f"layers.{i}.norm2.bias", _n2p(w[f"{block_prefix}LayerNorm_2/bias"]))
